    ...
    @if (!$quotes->isEmpty())
        ...
        <div class="grid grid-cols-6 gap-6">
            <div class="col-span-3">
                <label for="quote" class="block text-sm font-medium text-gray-700 dark:text-gray-400">quote #{{ $key+1 }}</label>
                <input type="text" name="quote" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:border-gray-700 dark:text-gray-400 dark:bg-gray-900 dark:placeholder-gray-300" wire:model="quotes.{{ $key }}.quote">
            </div>

            <div class="col-span-2">
                <label for="region" class="block text-sm font-medium text-gray-700 dark:text-gray-400">author #{{ $key+1 }}</label>
                <input type="text" name="region" id="region" autocomplete="address-level1" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:border-gray-700 dark:text-gray-400 dark:bg-gray-900 dark:placeholder-gray-300" wire:model="quotes.{{ $key }}.author">
            </div>
