<div class="p-2" x-data
        ...
        @foreach($systems as $key => $system)
            <div class="grid grid-cols-6 gap-6">
                <div class="col-span-5">
                    <label for="quote" class="block dark:text-gray-400 text-sm font-medium text-gray-700">operating system #{{ $key+1 }}</label>
                    <input type="text" name="quote" class="mt-1 block dark:border-gray-700 dark:text-gray-400 dark:bg-gray-900 dark:placeholder-gray-300 w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" wire:model="systems.{{ $key }}">
                </div>
    ...
    <div class="absolute bottom-0 inset-x-px">
        <div class="border-t border-gray-200 px-2 py-2 flex justify-between items-center space-x-3 sm:px-3 dark:border-gray-700">
            ...
            <div>
                <ul class="list-disc">
                    @if (!$systems->isEmpty())
                        @foreach($systems as $key => $system)
                            @error('systems.'.$key) <li class="text-red-800">{{ $message }}</li> @enderror
                        @endforeach
                    @endif
                </ul>
            </div>
