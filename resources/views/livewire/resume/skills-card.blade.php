            ...
            <ul class="text-lg ml-4">
                @if (!$skills->isEmpty())
                    @foreach($skills as $skill)
                        <li class="pb-4">
                            <a href="#" class="flex sm:flex-col md:flex-row justify-between">
                                <div class="border-b-4 border-solid transition duration-150 ease-in border-b-4 hover:border-{{ ($skill['level'] == 'advanced') ? 'gray' : (($skill['level'] == 'proficient') ? 'indigo' : (($skill['level'] == 'moderate') ? 'purple' : (($skill['level'] == 'basic') ? 'pink' : 'orange' ))) }}-500">
                                    {{ $skill['skill'] }}
                                </div>
                                <span class="float-right inline-flex items-center px-2.5 py-0.5 rounded-full h-6 text-xs font-medium bg-{{ ($skill['level'] == 'advanced') ? 'gray' : (($skill['level'] == 'proficient') ? 'indigo' : (($skill['level'] == 'moderate') ? 'purple' : (($skill['level'] == 'basic') ? 'pink' : 'orange' ))) }}-100 text-{{ ($skill['level'] == 'advanced') ? 'gray' : (($skill['level'] == 'proficient') ? 'indigo' : (($skill['level'] == 'moderate') ? 'purple' : (($skill['level'] == 'basic') ? 'pink' : 'orange' ))) }}-800">
