...
@elseif (count(explode(" ", \App\Models\Setting::where('key', '=', 'sitename')->first()->value)) >= 2)
    <div class="text-3xl font-bold rounded-full border border-gray-400 p-2 mr-2">
        {{ substr(explode(" ", \App\Models\Setting::where('key', '=', 'sitename')->first()->value)[0], 0, 1).' '.substr(explode(" ", \App\Models\Setting::where('key', '=', 'sitename')->first()->value)[1], 0, 1) }}
    </div>
@else
    <div></div>
@endif
